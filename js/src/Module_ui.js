(function() {
	...
	showStartGameUI=function()
	{
		...
		TweenLite.to(ui_newGame,0.8,{scale:.55,alpha:1,ease :Back.easeOut});
		...
		if(bladeHistory.length > 0){
			...
			ui_replayFruit.init(gameWidth*0.518,gameHeight*0.618,Infinity,textureObj.w,assetsManager.shadow,topContext);
			...
			TweenLite.to(ui_replayFruit,1,{scale:1,alpha:1,ease :Back.easeOut});
		}
	...
	showScoreTextUI=function(context, currScore)
	{
		...
		context.setTransform (1, 0, -.2, 1, 0, 0);
	...
	showScoreUI=function()
	{
		if (multiplayer){
			...
			ui_hudLeft.init(-30,gameHeight - 105,Infinity,assetsManager.hud,bottomLeftContext);
			ui_hudLeft.scale = .5
			
			ui_scoreIconLeft = particleSystem.createParticle(SPP.SpriteImage);
			ui_scoreIconLeft.regX=ui_scoreIconLeft.regY=0;
			ui_scoreIconLeft.init(10,gameHeight - 75,Infinity,assetsManager.score,bottomLeftContext);
			ui_scoreIconLeft.scale = .5;
			...
			ui_gameLifeLeft.init(165, bottomLeftContext.canvas.height-70,Infinity,ui_gamelifeTextureLeft,bottomLeftContext);
			
			ui_hudRight = particleSystem.createParticle(SPP.SpriteImage);
			ui_hudRight.regX=ui_hudRight.regY=0;
			ui_hudRight.init(-30,gameHeight - 105,Infinity,assetsManager.hud,bottomRightContext);
			ui_hudRight.scale = .5
